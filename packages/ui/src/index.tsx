...
export function PortalShell({ title, role, navigation, children }: { title: string; role: string; navigation: NavigationItem[]; children: ReactNode }) {
  return <div className="portal-shell"><aside><div className="portal-brand"><span>O</span><strong>OpportunityOS</strong></div><p className="portal-role">{role}</p><nav>{navigation.map(item => <a key={item.label} href={item.href} aria-disabled={item.enabled === false} className={item.enabled === false ? "disabled" : ""}>{item.label}{item.enabled === false && <small>受功能开关控制</small>}</a>)}</nav></aside><main><header><div><small>CORE PLATFORM</small><h1>{title}</h1></div><div className="tenant-chip">Test Tenant / Test Brand</div></header>{children}</main></div>;
}
